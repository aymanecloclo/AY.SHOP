...
const MenuItem = ({ to, children, className, onClick }) => (
  <li className={className}>
    <Link
      to={to}
      className="hover:text-gray-900 text-gray-600 font-semibold text-[15px] block lg:py-2"
      onClick={onClick}
    >
      {children}
    </Link>
  ...
  <div className="flex items-center px-16 py-3 bg-white">
    ...
    >
      <li className="max-lg:border-b max-lg:pb-4 px-3 lg:hidden ">
        <Link to="/">
          <img src={AYShopLogo} alt="logo" className="w-36" />
        </Link>
      </li>
      ...
      {menuData.map((item, index) => (
        <MenuItem
          key={index}
          to={item.to}
          className="max-lg:border-b max-lg:px-3 max-lg:py-3 lg:py-2 flex items-center"
        >
          {item.label}
        </MenuItem>
      ))}
      ...
      <li className="group max-lg:border-b max-lg:px-3 max-lg:py-3 lg:py-2 relative flex items-center">
        <Link
          to="/store"
          className="hover:text-gray-900 text-gray-600 font-semibold text-[15px] block"
        >
          Store
          <svg
          ...
          {Categories.map((category, index) => (
            <li key={index} className="border-b py-3">
              <Link
                ...
                className="hover:text-gray-900 text-gray-600 font-semibold text-[15px] block flex items-center gap-2"
              >
    ...
    <div className="mt-2 w-full lg:w-4/12 md:w-8/12 mx-auto lg:ms-28">
      <div className="relative flex items-center rounded-md bg-white pl-3 outline outline-1 -outline-offset-1 outline-gray-300 has-[input:focus-within]:outline-2 has-[input:focus-within]:-outline-offset-2 has-[input:focus-within]:outline-gray-900 px-0">
        <input
          ...
          name="price"
          id="price"
          className="block lg:w-[300px] w-full grow py-1.5 pl-1 pr-3 text-base text-gray-900 bg-white placeholder:text-gray-400 focus:outline focus:outline-0 sm:text-sm/6"
        />
        <div className="grid shrink-0 grid-cols-1 focus-within:relative lg:w-[80px] border">
          <select
            ...
            name="currency"
            aria-label="Currency"
            className="col-start-1 bg-white row-start-1 w-full appearance-none rounded-md py-1.5 pl-3 pr-7 text-base text-gray-500 placeholder:text-gray-400 focus:outline focus:outline-2 focus:-outline-offset-2 focus:outline-gray-900 sm:text-sm/6"
          >
            <option>All</option>
            <option>CAD</option>
            <option>EUR</option>
